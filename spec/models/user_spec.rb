...
describe User, type: :model do
  ...
  before(:each) do
    ...
    @review_10 = @book_4.reviews.create(review_title: "Not my favorite", rating: 3, review_text: "The love story was sad and had a bad ending", user_id: @user_12.id)
  ...
  it 'high_activity_users returns 3 highest active users' do
    hau = User.high_activity_users

    expect(hau.first).to eq @user_12
    expect(hau.last).to eq @user_8
  end

  it 'sorted_reviews sorts reviews ' do
    sr = User.high_activity_users.first.sorted_reviews({sort:'newest'})

    expect(sr.first).to eq @review_19
    expect(sr.last).to eq @review_5
  end
