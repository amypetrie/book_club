...
describe 'As a visitor' do
  ...
  describe "When I visit a user's page" do
    ...
    it 'I see a delete link for each review' do
      visit user_path(@user_12)

      within("#review_#{@review_19.id}") do
        expect(page).to have_content("Delete")
      end
    end

    it 'I can delete that review by clicking its delete link' do
      visit user_path(@user_12)

      within("#review_#{@review_19.id}") do
        click_on 'Delete'
        expect(current_path).to eq(user_path(@user_12))
      end

      visit user_path(@user_12)

      expect(page).to_not have_css("#review_#{@review_19.id}")
      expect(page).to have_css("#review_#{@review_5.id}")
    end
